...
namespace ProjPackageIntellisense
{
    ...
    internal class PackageCompletionSource : ICompletionSource
    {
        ...
        private IPackageFeedSearchJob<Tuple<string, SourceKind>> _nameSearchJob;
        private readonly IPackageSearchManager _searchManager;
        private readonly ITextBuffer _textBuffer;
        private IPackageFeedSearchJob<Tuple<string, SourceKind>> _versionSearchJob;
        ...
        private void ProduceNameCompletionSet()
        {
            ...
            Dictionary<string, SourceKind> packageLookup = new Dictionary<string, SourceKind>();

            foreach(Tuple<string, SourceKind> info in _nameSearchJob.Results)
            {
                if(!packageLookup.TryGetValue(info.Item1, out SourceKind existingInfo) || info.Item2 == SourceKind.Local)
                {
                    packageLookup[info.Item1] = info.Item2;
                }
            }

            foreach (KeyValuePair<string, SourceKind> entry in packageLookup)
            {
        ...
        {
            List<Completion> completions = new List<Completion>();
            Dictionary<string, SourceKind> iconMap = new Dictionary<string, SourceKind>();

            foreach (Tuple<string, SourceKind> info in _versionSearchJob.Results)
            {
                if (!iconMap.TryGetValue(info.Item1, out SourceKind existing) || existing != SourceKind.Local)
                {
                    iconMap[info.Item1] = info.Item2;
                }
            }

            foreach(KeyValuePair<string, SourceKind> entry in iconMap.OrderByDescending(x => SemanticVersion.Parse(x.Key)))
            {
                ImageMoniker moniker = KnownMonikers.NuGet;
